...
.controller('uWaterCtrl', ['$scope', '$http', '$q', 'uWaterFactory', function ($scope, $http, $q, uWaterFactory) {

    // Import variables and functions from service
    $scope.data = uWaterFactory.data;
	$scope.items = uWaterFactory.items;
	$scope.detailsItem = uWaterFactory.detailsItem;
	
    // initialize the service
    uWaterFactory.init($scope);

	// Show main view in the first column
	$scope.portalHelpers.showView('uWaterMain.html', 1);
	
	// This function gets called when user clicks an item in the list
	$scope.showDetails = function(item){
		// Make the item that user clicked available to the template
		$scope.detailsItem.value = item;		
		$scope.portalHelpers.showView('uWaterDetails.html', 2);
	}
	
}])
// Factory maintains the state of the widget
.factory('uWaterFactory', ['$http', '$rootScope', '$filter', '$q', function ($http, $rootScope, $filter, $q) {
...
.directive('uWaterDirectiveName', ['$http', function ($http) {
	return {
...
.filter('uWaterFilterName', function () {
